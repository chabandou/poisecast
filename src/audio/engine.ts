...
export class DenoiseEngine {
  ...
  async init(opts: {
    ...
    const worker = new Worker(
      new URL("./worker/denoise.worker.ts", import.meta.url),
      { type: "module" },
    );
    this.worker = worker;

    const readyPromise = new Promise<Extract<WorkerReply, { type: "ready" }>>(
      (resolve, reject) => {
    ...
    this._status = {
      ...
      frameSize: ready.frameSize,
    };
    this.frameSize = ready.frameSize;

    // Use a pre-created AudioContext if provided (created eagerly during user gesture),
    // otherwise create one here. On mobile, creating it after async work causes it to
    // start in 'suspended' state because the user gesture token has expired.
    this.ctx =
      opts.audioContext ??
      new AudioContext({
    ...
    await this.ctx.audioWorklet.addModule(
      new URL("./worklet/denoise-processor.js", import.meta.url),
    );
    ...
    this.worklet.port.onmessage = (evt: MessageEvent<WorkletFrameMsg>) => {
      ...
      this.worker?.postMessage(
        { type: "process", id: msg.id, audio: msg.audio },
        [msg.audio],
      );
    };
